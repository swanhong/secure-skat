...
func (netObj *Network) CollectiveBootstrap(cps *crypto.CryptoParams, ct *rlwe.Ciphertext, sourcePid int) {
	if netObj.GetPid() == 0 {
		return
	}

	if sourcePid > 0 {
		ct = netObj.BroadcastCiphertext(cps, ct, sourcePid)
	}

	minLevel, logBound, ok := mpckks.GetMinimumLevelForRefresh(128, cps.Params.DefaultScale(), max(1, netObj.GetNParty()-1), cps.Params.Q())
	if !ok || ct.Level() < minLevel {
		panic(fmt.Sprintf("CollectiveBootstrap: ciphertext level %d below required refresh level %d", ct.Level(), minLevel))
	}
	...
	crp := rfp.SampleCRP(cps.Params.MaxLevel(), netObj.sharedCRS())
	share := rfp.AllocateShare(ct.Level(), cps.Params.MaxLevel())
	if err := rfp.GenShare(cps.Sk, logBound, ct, crp, &share); err != nil {
		panic(err)
	}

	agg := rfp.AllocateShare(ct.Level(), cps.Params.MaxLevel())
	if netObj.GetPid() == netObj.GetHubPid() {
		for p := 1; p < netObj.GetNParty(); p++ {
			next := rfp.AllocateShare(ct.Level(), cps.Params.MaxLevel())
			if p == netObj.GetPid() {
				next = share
			} else if err := next.UnmarshalBinary(netObj.receiveBinaryValue(p)); err != nil {
				panic(err)
	...
	out := ct.CopyNew()
	if err := rfp.Finalize(ct, crp, agg, out); err != nil {
		panic(err)
	}
	*ct = *out
}
...
func SigmoidApprox(sourcePid int, net *Network, cryptoParams *crypto.CryptoParams, ctIn *rlwe.Ciphertext,
	...
	if intv.Degree == 0 {
		...
		for i := 0; i < len(cdDecode); i++ {
			cdOut[i] = real(Sigmoid(complex(cdDecode[i], 0)))
		}
	}
