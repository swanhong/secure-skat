...
// RandInt generates a random Int in [0, max-1] from a given random stream
func RandIntCustom(rnd io.Reader, max *big.Int) (n *big.Int) {
	var err error
	if n, err = rand.Int(rnd, max); err != nil {
		panic("error: crypto/rand/bigint")
	}
	return
}
