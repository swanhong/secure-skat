...
// MulSlots performs slot-wise multiplication in NTT domain.
// This is equivalent to element-wise multiplication of the evaluation points.
// For slot-wise operations, this should be used instead of MulCoeffsMontgomery
// which performs polynomial convolution.
func (r Ring) MulSlots(p1, p2, p3 Poly) {
	for i, s := range r.SubRings[:r.level+1] {
		mulslotsvec(p1.Coeffs[i], p2.Coeffs[i], p3.Coeffs[i], s.Modulus)
	}
}
