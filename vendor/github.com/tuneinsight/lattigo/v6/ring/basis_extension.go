...
func (be *BasisExtender) ModUpQtoP(levelQ, levelP int, polQ, polP Poly) {
	be.ModUpQtoPBuf(levelQ, levelP, polQ, polP, be.buffQ)
}

// ModUpQtoPBuf is like ModUpQtoP but uses the caller-supplied `scratch` as the
// intermediate buffer instead of the internal one. `scratch` must have at
// least levelQ+1 rows, each of length matching polQ/polP (multiple of 8).
// Length-agnostic — works on any ring.Poly shape where the SubRing per-
// coefficient ops are well-defined (which is length-multiple-of-8).
func (be *BasisExtender) ModUpQtoPBuf(levelQ, levelP int, polQ, polP, scratch Poly) {
	...
	ringQ.AddScalarBigint(polQ, QHalf, scratch)
	ModUpExact(scratch.Coeffs[:levelQ+1], polP.Coeffs[:levelP+1], be.ringQ, be.ringP, be.constantsQtoP[levelQ])
...
func (be *BasisExtender) ModUpPtoQ(levelP, levelQ int, polP, polQ Poly) {
	be.ModUpPtoQBuf(levelP, levelQ, polP, polQ, be.buffP)
}

// ModUpPtoQBuf is like ModUpPtoQ but uses the caller-supplied `scratch`
// instead of the internal buffer. See ModUpQtoPBuf.
func (be *BasisExtender) ModUpPtoQBuf(levelP, levelQ int, polP, polQ, scratch Poly) {
	ringQ := be.ringQ.AtLevel(levelQ)
	ringP := be.ringP.AtLevel(levelP)
	...
	ringP.AddScalarBigint(polP, PHalf, scratch)
	ModUpExact(scratch.Coeffs[:levelP+1], polQ.Coeffs[:levelQ+1], be.ringP, be.ringQ, be.constantsPtoQ[levelP])
...
func (be *BasisExtender) ModDownQPtoQ(levelQ, levelP int, p1Q, p1P, p2Q Poly) {
	be.ModDownQPtoQBuf(levelQ, levelP, p1Q, p1P, p2Q, be.buffQ, be.buffP)
}

// ModDownQPtoQBuf is like ModDownQPtoQ but uses caller-supplied `scratchQ`
// (Q-basis scratch, used for ModUpPtoQ intermediate) and `scratchP` (P-basis
// scratch, used internally by ModUpPtoQBuf). See ModUpQtoPBuf.
func (be *BasisExtender) ModDownQPtoQBuf(levelQ, levelP int, p1Q, p1P, p2Q, scratchQ, scratchP Poly) {
	ringQ := be.ringQ.AtLevel(levelQ)
	modDownConstants := be.modDownConstantsPtoQ[levelP]

	be.ModUpPtoQBuf(levelP, levelQ, p1P, scratchQ, scratchP)

	for i, s := range ringQ.SubRings[:levelQ+1] {
		s.SubThenMulScalarMontgomeryTwoModulus(scratchQ.Coeffs[i], p1Q.Coeffs[i], s.Modulus-modDownConstants[i], p2Q.Coeffs[i])
	}
}
...
func (decomposer *Decomposer) DecomposeAndSplit(levelQ, levelP, nbPi, BaseRNSDecompositionVectorSize int, p0Q, p1Q, p1P Poly) {
	...
	// N-agnostic: iterate over actual slice length, not ringQ.N(). This lets
	// callers pass polys whose Coeffs[i] is longer than ringQ.N() (e.g. flat
	// multivariate layouts of length n1·ringQ.N()), as long as the length is
	// a multiple of 8. For standard callers, len(p0Q.Coeffs[0]) == ringQ.N().
	N := len(p0Q.Coeffs[0])
