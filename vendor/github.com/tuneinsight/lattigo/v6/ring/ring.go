...
func NewRingWithCustomNTT(N int, ModuliChain []uint64, ntt func(*SubRing, int) NumberTheoreticTransformer, NthRoot int) (r *Ring, err error) {
	...
	for i := 0; i < len(ModuliChain); i++ {
		prod.Mul(prod, new(big.Int).SetUint64(ModuliChain[i]))
		// store a deep copy; don't reuse prod pointer
		r.ModulusAtLevel[i] = new(big.Int).Set(prod)
	}
