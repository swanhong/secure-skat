...
// mulslotsvec performs slot-wise multiplication: p3[i] = p1[i] * p2[i] mod q
// This is element-wise multiplication in the NTT domain, which corresponds
// to slot-wise multiplication in the coefficient domain.
// Uses Barrett reduction to handle large numbers safely.
func mulslotsvec(p1, p2, p3 []uint64, modulus uint64) {
	N := len(p1)
	// Generate Barrett reduction constant for this modulus
	brc := GenBRedConstant(modulus)
	...
	for j := 0; j < N; j = j + 8 {
		...
		// Element-wise multiplication with Barrett reduction to prevent overflow
