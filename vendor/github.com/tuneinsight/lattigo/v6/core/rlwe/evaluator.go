	...
	if !utils.IsNil(evk) {
		if galEls := evk.GetGaloisKeysList(); len(galEls) != 0 {
			...
			for _, galEl := range galEls {
				// For 3N rings, use ring-specific index computation
				if ringQ.Type() == ring.Matrix {
					if AutomorphismIndex[galEl], err = ringQ.AutomorphismNTTIndex3N(galEl); err != nil {
						panic(err)
					}
				} else {
...
func (eval Evaluator) CheckAndGetGaloisKey(galEl uint64) (evk *GaloisKey, err error) {
	...
	if _, ok := eval.automorphismIndex[galEl]; !ok {
		...
		// For 3N rings, use ring-specific index computation
		if ringQ.Type() == ring.Matrix {
			if eval.automorphismIndex[galEl], err = ringQ.AutomorphismNTTIndex3N(galEl); err != nil {
				panic(err)
			}
		} else {
			if eval.automorphismIndex[galEl], err = ring.AutomorphismNTTIndex(eval.params.N(), ringQ.NthRoot(), galEl); err != nil {
				panic(err)
			}
		}
	}
