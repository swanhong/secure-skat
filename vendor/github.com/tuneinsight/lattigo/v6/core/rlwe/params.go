...
// ParameterLiteral for 3N rings.
// Dimension is specified as 2^Order3 * 3^Order3.
type ParametersLiteral3N struct {
	Order2          int                         `json:",omitempty"`
	Order3          int                         `json:",omitempty"`
...
type Parameters struct {
	...
	// Galois generators are dependent to the ring dimension.
	galoisGenRow uint64
	galoisGenCol uint64
}
...
func NewParameters(logn int, q, p []uint64, xs, xe DistributionLiteral, ringType ring.Type, defaultScale Scale, NTTFlag bool) (params Parameters, err error) {
	return NewParameters3N(logn, 0, q, p, xs, xe, ringType, defaultScale, NTTFlag)
}

// NewParameters3N returns a new set of generic RLWE parameters supporting 3N rings (N = 2^a * 3^b).
...
func NewParameters3N(order2, order3 int, q, p []uint64, xs, xe DistributionLiteral, ringType ring.Type, defaultScale Scale, NTTFlag bool) (params Parameters, err error) {
	...
	dim := DimSpec{Order2: order2, Order3: order3}
	theN = dim.Dimension()
	size := bits.Len64(uint64(theN))

	if err = checkSizeParams(size); err != nil {
		return Parameters{}, err
	}

	g1, g2, err := ComputeGaloisGenerators(theN)
	if err != nil {
		return Parameters{}, err
	}
	...
	case ring.Ternary, ring.DiscreteGaussian:
		params.xs = NewDistribution(xs.(ring.DistributionParameters), dim)
	default:
	...
	switch xe := xe.(type) {
	case ring.Ternary, ring.DiscreteGaussian:
		params.xe = NewDistribution(xe.(ring.DistributionParameters), dim)
	default:
...
func NewParametersFromLiteral(paramDef interface{}) (params Parameters, err error) {

	switch paramDef := paramDef.(type) {
	// Usual literals
	case ParametersLiteral:
		if paramDef.Xs == nil {
		...
		if paramDef.LogDefaultScale != 0 && paramDef.DefaultScale.Cmp(Scale{}) != 0 {
			return Parameters{}, fmt.Errorf("rlwe.NewParametersFromLiteral: both LogDefaultScale and DefaultScale fields are set")
		}
		if paramDef.DefaultScale.Cmp(Scale{}) == 0 && paramDef.LogDefaultScale != 0 {
			paramDef.DefaultScale = NewScale(math.Exp2(float64(paramDef.LogDefaultScale)))
		}
		...
		if paramDef.LogQ != nil || paramDef.LogP != nil {
			...
			case ring.Matrix:
				return Parameters{}, fmt.Errorf("rlwe.NewParametersFromLiteral: matrix ring type not supported")
			}
		}
		...
		return NewParameters(logN, q, p, paramDef.Xs, paramDef.Xe, paramDef.RingType, paramDef.DefaultScale, paramDef.NTTFlag)

	// Literals for 3N rings
	case ParametersLiteral3N:
		if paramDef.Xs == nil {
			paramDef.Xs = DefaultXs
		}
		...
		if paramDef.LogQ != nil || paramDef.LogP != nil {
			...
			default:
				return Parameters{}, fmt.Errorf("rlwe.NewParametersFromLiteral: invalid ring type")
			}
		}
		// Use the user-provided primes if specified.
		if q == nil {
	...
	default:
		return Parameters{}, fmt.Errorf("rlwe.NewParametersFromLiteral: invalid literal type %T", paramDef)
	}
}
...
func (p Parameters) StandardParameters() (pci Parameters, err error) {
	...
	case ring.ConjugateInvariant:
		pci = p
		pci.n.Order2 = p.n.Order2 + 1
		pci.n.Order3 = p.n.Order3
...
// ParametersLiteral3N returns the ParametersLiteral3N of the target Parameters.
func (p Parameters) ParametersLiteral3N() ParametersLiteral3N {
...
func (p Parameters) DimSpec() DimSpec {
	return p.n
}

// N returns the ring degree
func (p Parameters) N() int {
	return p.n.Dimension()
}
...
// GaloisElementRow takes an integer k and returns p.GaloisGenRow^{k} mod NthRoot.
func (p Parameters) GaloisElementRow(k int) uint64 {
	/* #nosec G115 -- implicit reduction modulo 2^64 */
	return ring.ModExp(p.galoisGenRow, uint64(k)&(p.ringQ.NthRoot()-1), p.ringQ.NthRoot())
}

// GaloisElementRow takes an integer k and returns p.GaloisGenCol^{k} mod NthRoot.
func (p Parameters) GaloisElementCol(k int) uint64 {
	/* #nosec G115 -- implicit reduction modulo 2^64 */
	return ring.ModExp(p.galoisGenCol, uint64(k)&(p.ringQ.NthRoot()-1), p.ringQ.NthRoot())
}

// GaloisGenRow returns the Galois generator g1 for row rotations.
func (p Parameters) GaloisGenRow() uint64 {
	return p.galoisGenRow
}

// GaloisGenCol returns the Galois generator g2 for column rotations.
func (p Parameters) GaloisGenCol() uint64 {
	return p.galoisGenCol
}
...
func (p Parameters) ModInvGaloisElement(galEl uint64) uint64 {
	if p.ringType == ring.Matrix {
		// If ringType is Matrix, compute the modular inverse of galEl modulo 3*N (N = NthRoot)
		// Use ModInvExtendedEuclidean from utils.go
		N := p.ringQ.NthRoot()
		modulus := 3 * N
		galElInv, err := InvMod(galEl, modulus)
		if err != nil {
			panic(err)
		}
		return galElInv
...
// // GaloisGenRow returns the Galois generator g1 for row rotation in Matrix CKKS.
// func (p Parameters) GaloisGenRow() uint64 {
// 	return p.galoisGenRow
// }

// // GaloisGenCol returns the Galois generator g2 for column rotation in Matrix CKKS.
// func (p Parameters) GaloisGenCol() uint64 {
// 	return p.galoisGenCol
// }
...
// LATER Marshal and Unmarshal needs to cover 3N rings
...
// MAYBE Modify to use existing machinery
