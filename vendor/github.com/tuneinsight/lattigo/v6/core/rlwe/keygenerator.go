...
func (kgen KeyGenerator) GenGaloisKey(galEl uint64, sk *SecretKey, gk *GaloisKey) {
	...
	if ringQ.Type() == ring.Matrix {
		// 3N ring: use NTT domain permutation (same as power-of-2)
		index, err := ringQ.AutomorphismNTTIndex3N(galElInv)
		if err != nil {
			panic(err)
		}
		...
		if ringP != nil {
			indexP, errP := ringP.AutomorphismNTTIndex3N(galElInv)
			if errP != nil {
				panic(errP)
			}
			ringP.AutomorphismNTTWithIndex(skIn.P, indexP, skOut.P)
		}
	} else {
		// Power-of-2: use NTT domain permutation (works correctly)
		...
		if ringP != nil {
			indexP, errP := ring.AutomorphismNTTIndex(ringP.N(), ringP.NthRoot(), galElInv)
			if errP != nil {
				panic(errP)
			}
			ringP.AutomorphismNTTWithIndex(skIn.P, indexP, skOut.P)
