...
// RotationCountHook is an optional callback for counting automorphism-based
// rotations during tests. It should be nil in production usage.
var RotationCountHook func(kind string)
...
func (eval Evaluator) countAutomorphism(galEl uint64, hoisted bool) {
	if RotationCountHook == nil {
		return
	}

	kind := "rot"
	if galEl == eval.GetParameters().GaloisElementOrderTwoOrthogonalSubgroup() {
		kind = "conj"
	}
	if hoisted {
		kind += "-hoisted"
	}
	RotationCountHook(kind)
}

// Automorphism applies the automorphism corresponding to galEl and records the
// call in RotationCountHook when enabled.
func (eval Evaluator) Automorphism(op0 *rlwe.Ciphertext, galEl uint64, opOut *rlwe.Ciphertext) (err error) {
	eval.countAutomorphism(galEl, false)
	return eval.Evaluator.Automorphism(op0, galEl, opOut)
}

// AutomorphismHoisted applies the hoisted automorphism corresponding to galEl
// and records the call in RotationCountHook when enabled.
func (eval Evaluator) AutomorphismHoisted(levelQ int, op0 *rlwe.Ciphertext, c1DecompQP []ringqp.Poly, galEl uint64, opOut *rlwe.Ciphertext) (err error) {
	eval.countAutomorphism(galEl, true)
	return eval.Evaluator.AutomorphismHoisted(levelQ, op0, c1DecompQP, galEl, opOut)
}

// AutomorphismHoistedLazy applies the hoisted lazy automorphism corresponding
// to galEl and records the call in RotationCountHook when enabled.
func (eval Evaluator) AutomorphismHoistedLazy(levelQ int, op0 *rlwe.Ciphertext, c1DecompQP []ringqp.Poly, galEl uint64, opOut *rlwe.Element[ringqp.Poly]) (err error) {
	eval.countAutomorphism(galEl, true)
	return eval.Evaluator.AutomorphismHoistedLazy(levelQ, op0, c1DecompQP, galEl, opOut)
}
