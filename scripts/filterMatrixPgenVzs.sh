...
if [ -z "${ROW_FILT_FILE}" ]
then
    plink2 --pfile "${PGEN_PREFIX}" vzs --extract "${OUT_FILE}.snps.txt" --indiv-sort none --make-bed --out "${OUT_FILE}"
else
    plink2 --pfile "${PGEN_PREFIX}" vzs --keep "${ROW_FILT_FILE}" --extract "${OUT_FILE}.snps.txt" --indiv-sort none --make-bed --out "${OUT_FILE}"
fi
EXTRA_ARGS=""
